...
import {
	recipientTypes,
	recipientTypeGroups
} from "dataMapping/search/recipientType";
...
export default class RecipientType extends React.Component {
	componentDidUpdate(prevProps) {
		if (
			this.props.dirtyFilters &&
			prevProps.dirtyFilters !== this.props.dirtyFilters
		) {
			if (this.hint) {
				this.hint.showHint();
			}
	...
	render() {
		const checkboxTypes = this.props.recipientTypeMapping.map((type, index) => (
			<PrimaryCheckboxType
				{...type}
				{...this.props}
				key={index}
		...
		return (
			<div className="filter-item-wrap">
				<div className="checkbox-type-filter">
					<ul className="checkbox-types">{checkboxTypes}</ul>
					<SubmitHint
						ref={component => {
							this.hint = component;
						}}
